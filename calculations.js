...
function calculateSurfaceTemperatures(latitude, forecastHigh, forecastLow) {
    // --- 物理常數 ---
    const GSC = 1367; [cite_start]// 太陽常數 W/m^2 [cite: 1]
    const SIGMA = 5.67e-8; [cite_start]// 史蒂芬-波茲曼常數 W/m^2K^4 [cite: 1]
    ...
    const cloudCoefficient = delta_T_observed / delta_T_max; [cite_start]// 雲遮係數 [cite: 1]

    [cite_start]// 公式4: 計算實際太陽入射輻射量 (Rs_observed) [cite: 1]
    const Rs_observed = Rs_max * cloudCoefficient;
    
    [cite_start]// 公式5: 為每種鋪面計算表面溫度 [cite: 1]
    return pavements.map(pavement => {
        ...
        [cite_start]// 將克氏溫標轉換為攝氏溫標 [cite: 1]
